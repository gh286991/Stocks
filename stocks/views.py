...
def get_stocks_api(request):
    ...
    # ---------------------策略參數------------------
    values = Inputdata['MarkCap']
    freecashflow = Inputdata['FCF']
    shareholder = Inputdata['ROE']
    grows = Inputdata['OPM']
    Revenue = Inputdata['PSR']
    rsvs = Inputdata['RSV']
    pricess = Inputdata['price']   

    print('values' ,type(values))
    # -----------------------參數---------------------
    data = Data()
    Sda = Inputdata['startD'].split('-')
    start_date = datetime.date(int(Sda[0]),int(Sda[1]),int(Sda[2]))
    ends = Inputdata['endD'].split('-')
    end_date = datetime.date(int(ends[0]),int(ends[1]),int(ends[2]))
    strategy = mystrategy2
    data = data
    hold_days = Inputdata['period']
    stocks = mystrategy2(data,values,freecashflow,shareholder,grows,Revenue,rsvs,pricess)
    weight='average'
    benchmark=None
    stop_loss=None
