...
def table_latest_date(conn, table):
    cursor = conn.execute('SELECT * FROM ' + table + ' ORDER BY date DESC LIMIT 1;')
    return datetime.datetime.strptime(list(cursor)[0][1], '%Y-%m-%d %H:%M:%S') 

def table_earliest_date(conn, table):
    cursor = conn.execute('SELECT * FROM ' + table + ' ORDER BY date ASC LIMIT 1;')
    return datetime.datetime.strptime(list(cursor)[0][1], '%Y-%m-%d %H:%M:%S') 
...
def widget(conn, table_name, crawl_func, range_date):
    ...
    if table_exist(conn, table_name):
        ...
        label = widgets.Label(table_name + ' (No table found)')
